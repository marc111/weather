 ...
 */
(function () {
	if (typeof fs121 == "undefined") {
		return;
	}
	var phy = fs121.phy,
		util=fs121.util,
		tmutil=fs121.tmgram.util,
		g2dlnp = fs121.g2dlnp,
		tlnpplugin=fs121.namespace("tmgram.tlnpplugin");
	//以风向为X轴
	//@param order
	//@param min
	//@param max
	...
	fs121.tmgram.PLUGIN_GROUP={
		...
		//	]
		STD:[
					['Framework'],['Qs'],['DryAdiabat'],['MoistAdiabat'],['QsLabel'],['DryAdiabatLabel'],['MoistAdiabatLabel'],['XaxisLabel',{position:'bottom'}],['XaxisLabel_K',{'t_ref':'XaxisLabel'}],['YaxisLabel',{mode:"tlnp"}],['HeightCoor'],['Glass'],
					['StratificationCurve',{wrap:true}],['StateCurve'],['Wind',{wrap:true}],
					['Idx',{position:'rt',fillStyle:"rgba(100%,100%,100%,0.9)",items:[{color:'black',lineWidth:2,label:'风速'},{color:'#777',lineWidth:1,label:'风向'}]}],
					['Mark']
			]
		,P_H:[
					['Framework'],['XaxisLabel',{position:'top'}],['XaxisLabel-h'],['YaxisLabel'],['HeightCoor'],['Glass'],['PressHeightCurve'],['StratificationCurve'],['Mark']
			]
		,LCL:[
					['Framework'],['Qs'],['DryAdiabat'],['MoistAdiabat'],['XaxisLabel',{position:'bottom'}],['XaxisLabel_K',{'t_ref':'XaxisLabel'}],['YaxisLabel'],['Glass',{fillStyle:"rgba(255,238,119,0.8)"}],['Mark'],
					['LCL'],['StratificationCurve'],['StateCurve'],['LCL.label'],['HeightCoor']
			]
		,CCL:[
					['Framework'],['Qs'],['DryAdiabat'],['MoistAdiabat'],['XaxisLabel',{position:'bottom'}],['XaxisLabel_K',{'t_ref':'XaxisLabel'}],['YaxisLabel'],['Glass',{fillStyle:"rgba(255,238,119,0.8)"}],['Mark'],
					['CCL'],['StratificationCurve'],['CCL.label'],['HeightCoor']
			]
		,SpecialTempLayer:[
					['Framework'],['Qs'],['DryAdiabat'],['MoistAdiabat'],['QsLabel'],['DryAdiabatLabel'],['MoistAdiabatLabel'],['XaxisLabel',{position:'bottom'}],['XaxisLabel_K',{'t_ref':'XaxisLabel'}],['YaxisLabel'],['Wind'],['StateCurve'],['Glass',{fillStyle:"rgba(255,238,119,0.6)"}],['Mark'],
					['StratificationCurve'],['SpecialTempLayer'],['SpecialTempLayer.label'],['HeightCoor']
			]
		,VDISTRIBUTION:[
					['Framework',{color_background:'white'}],['XaxisLabel'],['YaxisLabel'],['HeightCoor',{mode:'standard'}],['Mark']
			]
		,QCurve:[
					['CanvasReset',{tmin:0,tmax:40}],['Framework',{color_background:'white'}],['XaxisLabel',{unit:'g/kg',step:5,position:'bottom'}],['XaxisLabel#2',{unit:'g/kg',step:5,position:'top',unit_position:'left'}],['YaxisLabel'],['HeightCoor',{mode:'standard'}],['Mark'],['QCurve']
			]
		,RhCurve:[
					['CanvasReset',{tmin:0,tmax:100}],['Framework',{color_background:'white'}],['XaxisLabel',{unit:'%',position:'bottom'}],['XaxisLabel#2',{unit:'%',position:'top',unit_position:'left'}],['YaxisLabel'],['HeightCoor',{mode:'standard'}],['Mark'],['RhCurve']
			]
		,V_3theta:[
					['CanvasReset',{tmin:-20,tmax:120}],['Framework',{color_background:'white'}],['XaxisLabel',{position:'bottom'}],['XaxisLabel_K',{'t_ref':'XaxisLabel'}],['YaxisLabel'],['HeightCoor',{mode:'standard'}],['Mark'],['V_3theta'],
		...
		,V_WindSpeedProfile:[
					['CanvasReset',{tmin:0,tmax:80}],['Framework',{color_background:'white'}],
	...
	function PluginMnt(canv,dataMnt){
		...
		function register(pid,funcname){
			var id=pid.slice(0),n=pid.indexOf("#");
			if(n!=-1)
				id=id.substring(0,n);
			var p=this.find(pid);
			if(p==null){
				p=fs121.tmgram.tlnpplugin[id].create(this.canv);
				p._pluginMnt=this;
				p.id=pid;
				p.canv=this.canv;
		...
		function draw(opt){
			if(this.dataMnt.data){
				for(var i in this._plugins){
					var p=this._plugins[i].obj,funcname=this._plugins[i].funcname;
	...
	var Tlnpplugin={
		...
		,draw_tplabels: function(pts,opt,ctx_opt){
			...
			pts=pts.map(function(v,i){
					switch(i%3){
						case 0: return v;
						case 1: return canv.coor_x(v);
						case 2: return canv.coor_y(v);
					}});
...
//==================================================== 等比湿线 及 标值 ====================================================
	...
	var Qs=create({
		...
		//override
		,draw:function(opt){
			var canv=this.canv, settings=canv.settings;
	...
	var Ema50QsLabel=create2(Qs,{
		...
		,create:function(canv){
			...
			switch(diagram_mode){
				case "ema200" : return Object.create(Ema200QsLabel);
				default: return Object.create(Ema50QsLabel);
			}
		}
		,draw:function(opt){
			var canv=this.canv, settings=canv.settings, lbs=[];
			var p=475;//指定的水平气压线值
...
//==================================================== 干绝热线 及 标值 ====================================================
	...
	var Ema50DryAdiabat=create({
		...
		,create:function(canv){
			var diagram_mode=canv.diagram_mode;
			switch(diagram_mode){
				case "ema200" : return Object.create(Ema200DryAdiabat);
				default: return Object.create(Ema50DryAdiabat);
			}
		}
		//override
		,draw:function(){
	...
	var Ema50DryAdiabatLabel=create2(Ema50DryAdiabat,{
		...
		,create:function(canv){
			var diagram_mode=canv.diagram_mode;
			switch(diagram_mode){
				case "ema200" : return Object.create(Ema200DryAdiabatLabel);
				default: return Object.create(Ema50DryAdiabatLabel);
			}
		}
		//制作标记位置数据
...
//==================================================== 湿绝热线 及 标值 ====================================================
	...
	var Ema50MoistAdiabat=create({
		...
		,create:function(canv){
			var diagram_mode=canv.diagram_mode;
			switch(diagram_mode){
				case "ema200" : return Object.create(Ema200MoistAdiabat);
				default: return Object.create(Ema50MoistAdiabat);
			}
		}
		//等θse线取值,由继承对象中提供
	...
	var Ema50MoistAdiabatLabel=create2(Ema50MoistAdiabat,{
		...
		,create:function(canv){
			var diagram_mode=canv.diagram_mode;
			switch(diagram_mode){
				case "ema200" : return Object.create(Ema200MoistAdiabatLabel);
				default: return Object.create(Ema50MoistAdiabatLabel);
			}
		}
		,draw:function(){
			var canv=this.canv;
...
//==================================================== 框架插件 ====================================================
	var Framework=create({
		...
		,draw:function(){
			var canv=this.canv, ctx=canv.ctx, settings=canv.settings,ts=this.getSettings();
...
//==================================================== X-轴定制插件，可作为其它x轴插件原型 ====================================================
	var XaxisLabel_cust=create({
		...
		,_init:function(){
			var canv=this.canv, settings=canv.settings,ts=this.getSettings();
		...
		,_drawIt:function(min,max,coor){
			var canv=this.canv, settings=canv.settings,ts=this.getSettings()
				,y=settings.padding_top,h=settings.viewport_height
			...
			for(var t=min;t<=max;t+=ts.step){
				var cx=coor(t),cy=ts.position=='top'? y:y+h+1; //当作为下x-轴标尺时，加空隙"1"
				pts.push(t,cx,cy);
			}
			pts=reorder(pts);
			tofix(pts);
			...
			function tofix(pts){
				...
				var ih=1,wh=canv.ctx.measureText(pts[ih-1]).width	//队列头
					,it=pts.length-2,wt=canv.ctx.measureText(pts[it-1]).width;	//队列尾
...
//==================================================== x(温度)-轴标值插件 ====================================================
	var XaxisLabel=create2(XaxisLabel_cust,{
		...
		,draw:function(){
			var canv=this.canv,ts=this.getSettings()
				,min=Math.ceil(ts.min/10)*10,max=Math.floor(ts.max/10)*10
...
//==================================================== x(绝对温标)-轴标值插件 ====================================================
	var XaxisLabel_K=create2(XaxisLabel_cust,{
		...
		,_init:function(){
			var ts=this.getSettings(),refts=this.findSibling(ts.t_ref).getSettings()
		...
		,draw:function(){
			var canv=this.canv, settings=canv.settings,ts=this.getSettings()
				,min=Math.ceil(ts.min/10)*10,max=Math.floor(ts.max/10)*10
				,coor_x2=canv.coor_x.bind(canv),coor_x=function(T){return coor_x2(phy.t(T));}
				this._drawIt(min,max,coor_x);
		}
	});
//==================================================== y-轴标值(Ema50-1050)插件 ====================================================
	var Ema50YaxisLabel=create({
		...
		,create:function(canv){
			var diagram_mode=canv.diagram_mode;
			switch(diagram_mode){
				case "ema200" : return Object.create(Ema200YaxisLabel);
				default: return Object.create(Ema50YaxisLabel);
			}
		}
		,draw:function(){
			var canv=this.canv, settings=canv.settings,
				x=settings.padding_left,y=settings.padding_top,w=settings.viewport_width,
				opt={label_color:this._color_gridlines,font_height:this._label_font_height},
...
//==================================================== 毛玻璃插件 ====================================================
	var Glass=create({
		...
		,draw:function(){
			var canv=this.canv, ctx=canv.ctx, settings=canv.settings,
				x=settings.padding_left,y=settings.padding_top,w=settings.viewport_width,h=settings.viewport_height;
			ctx.beginPath();
...
//==================================================== 标识插件 ====================================================
	var Mark=create({
		...
		,draw:function(){
			var ts=this.getSettings(),canv=this.canv,settings=canv.settings
...
//==================================================== 高空风插件 ====================================================
	var Wind=create({
		...
		,draw:function(){//描绘高空风符号
			var ts=this.getSettings(),core_radius=0,shaftLength=ts.shaftLength,ctx_opt={strokeStyle:ts.color}
				,canv=this.canv,ctx=canv.getContext();
			if(ts.xOn=='t'){
				var wt=this.dataMnt.getSeries('w&t',true)
				for(var i=0;i<wt.length;i+=4){
					var wd=wt[i],wf=wt[i+1],t=wt[i+2],p=wt[i+3];
				...
				for(var i=0;i<wt.length;i+=3){
					var wd=wt[i],wf=wt[i+1],p=wt[i+2];
			...
			function draw(wd,wf,t,p,wrap){
				...
				for(var j=0;j<tps.length;j+=2){
					var x=tps[j],y=tps[j+1];
					fs121.wsymbol.drawWind(ctx,wd,wf,{x:x,y:y},core_radius,shaftLength,false,ctx_opt);
				}
			}
		}
	});
...
//==================================================== 图例插件 ====================================================
	var Legend=create({
		...
		,draw:function(){//描绘
			var ts=this.getSettings(),canv=this.canv,settings=canv.settings,ctx=canv.ctx
				,wh=calcWH(),frame=canv.legend_frame(util.extend({},ts,{width:wh[0],height:wh[1]})),x=frame.X(0),y=frame.Y(0);	
			ctx.save();
			frame.draw();
			...
			//计算lengend的长和宽
			function calcWH(){
				var len=ts.items.length,w=0,h=len*ts.item_height+(len-1)*ts.item_vSpan;
				for(var i=0;i<len;i++){
			...
			function drawItem(x,y,item){
				var x0,y0,x1,y1;
				switch(item.type){
					case "|":	x0=x1=x+ts.item_icon_width/2; y0=y; y1=y0+ts.item_height; break;
					case "\\":	x0=x,x1=x+ts.item_icon_width; y0=y; y1=y+ts.item_height; break;
					case "/":	x0=x,x1=x+ts.item_icon_width; y1=y; y0=y+ts.item_height; break;
					default :	x0=x; y0=y1=y+ts.item_height/2; x1=x+ts.item_icon_width;//"-"
				}
...
//==================================================== 指数插件 ====================================================
	var Idx=create({
		...
		,draw:function(){//描绘
			var ts=this.getSettings(),canv=this.canv,settings=canv.settings,ctx=canv.ctx,items=this._make_idx_items(),
				wh=calcWH(items),frame=canv.legend_frame(util.extend({},ts,{width:wh[0],height:wh[1]})),x=frame.X(0),y=frame.Y(0);	
			ctx.save();
			frame.draw();
			ctx.textBaseline="middle";
			drawItems(x,y,items);
			...
			function calcWH(items){
				var len=items.length,w=0,h=len*ts.item_height+(len-1)*ts.item_vSpan;
				for(var i=0;i<len;i++){
					var item=items[i];
					ctx.font=ts.item_font;
		...
		,_make_idx_items:function(){
			var idx=phy.index,items0={SI:'SI沙氏指数',TT:'TT全总指数',A:'A指数',K:'K指数',KY:'KY山崎指数',SWEAT:'SWEAT强天气威胁'},items=[],dm=this.dataMnt,la=dm.getSta().la
				,t500=dm.calcT(500),td500=dm.calcTd(500),t700=dm.calcT(700),td700=dm.calcTd(700),t850=dm.calcT(850),td850=dm.calcTd(850),W500=dm.getW(500),W850=dm.getW(850)
				,wd500=W500!=null? W500[0]:null,wf500=W500!=null? W500[1]:null,wd850=W850!=null? W850[0]:null,wf850=W850!=null? W850[1]:null
				,T500=phy.T(t500),Td500=phy.T(td500),T850=phy.T(t850),Td850=phy.T(td850)
			...
			if((KY_TA=idx.KY_TA(la,wd500,wf500,wd850,wf850))!=null && (KY=idx.KY(SI,KY_TA,T850,Td850))!=null){
				var den=1+T850-Td850,s1=KY_TA>SI? 'TA>SI':'TA<=SI'
...
//==================================================== 压高曲线插件 ====================================================
	var PressHeightCurve=create({
		...
		,draw:function(){//描绘层结曲线
			//this.canv,this.dataMnt由外部设置
			var canv=this.canv, settings=canv.settings,line_points=[], pts=this.dataMnt.getSeries('h',false), ctx_opt={strokeStyle:'black',lineWidth:3}
				,min=0,max=canv.diagram_mode=="ema200"? 12.5 : 25
				,coor_x=coor_x_desc.bind(null,min,max,canv);
			if(pts==null)
				return;
			for(var j=0,jlen=pts.length;j<jlen;j+=2){//全层/半层的低层曲线段端点
				var t=pts[j],p=pts[j+1];
...
//==================================================== 画折线插件 ====================================================
	var Polyline=create({
		...
		,draw: function(){
			var canv=this.canv, ctx=canv.getContext(), ts=this.getSettings(),	pts=canv.toPx(ts.pts);
...
//==================================================== 状态曲线插件 ====================================================
	var StateCurve=create({
		...
		,draw:function(opt){//描绘层结曲线
			var canv=this.canv, orgin=this.getSettings().orgin;
			//this.canv,this.dataMnt由外部设置
			var d4f=this.dataMnt.getData4State(orgin); //获取计算状态曲线所需要的数据
			if(d4f!=null){
				var T=d4f.Tps[0],Td=d4f.Td,p=d4f.Tps[1],thetase0=phy.thetase(T,Td,p),lcl=phy.LCL(T,Td,p)
			...
			function fill(Ltp,Ls,lfe,thetase){
				...
				var res=g2dlnp.mark2polyline(Ltp,Ls)
					,ctx_opt_p={fillStyle:'red',globalAlpha:0.35},ctx_opt_n={fillStyle:'blue',globalAlpha:0.35};
				for(var i=0,len=res.length;i<len;i+=3){
					var s=res[i],L0=res[i+1],L1=res[i+2];
					if(L0!=null && L1!=null){
	...
	var HeightCoor=create({
		...
		,draw:function(){
			var canv=this.canv,settings=canv.settings,hp=this.dataMnt.getSeries('h',true),
			...
			function drawScale(pts){//绘画刻度
				for(var i=0,len=pts.length;i<len;i+=3){
					var x=pts[i+1],y=pts[i+2];
					canv.draw_polyline([x-4,y,x,y],scale_ctx_opt);
				}
			}
			function orgin(L,position){//构建实测位高标值数据
				var pts=[],x=settings.viewport_right;//settings.padding_left+settings.viewport_width;
				for(var i=0;i<L.length;i+=2){
					var p=L[i+1],h=Math.round(L[i]/10); //高度单位用位势十米
			...
			function standard(L,position){//构建标准化位高标值数据
				var pts=[],h=Math.ceil(L[0]/1000)*1000,x=settings.viewport_right
					,p=settings.pmax
...
//==================================================== 抬升凝结高度LCL插件 ====================================================
	var LCL=create({
		...
		,draw:function(opt){
			...
			if(d4f!=null){
				var T=d4f.Tps[0],Td=d4f.Td,p=d4f.Tps[1],lcl=phy.LCL(T,Td,p),theta0=phy.theta(T,p),thetase0=phy.thetase(T,Td,p);
				//qs
				...
				if(lfe.LFC){
					var l_t=lfe.LFC[0],l_p=lfe.LFC[1];
				...
				if(lfe.EL){
					var l_t=lfe.EL[0],l_p=lfe.EL[1],l_tmin,l_tmax,l_ta,tAlign;
				...
				if(lfe.EAL){
					var cp=lfe.EAL,l_t0=cp[0],l_t1=cp[2],l_p=cp[1];
...
//==================================================== 抬升凝结高度LCL插件 ====================================================
	var CCL=create({
		...
		,draw:function(){
			//this.canv,this.dataMnt由外部设置
			var canv=this.canv,settings=canv.settings,dataMnt=this.dataMnt
				,d4f=dataMnt.getData4State('surf'); //获取计算状态曲线所需要的数据
			if(d4f!=null){
				var T0=d4f.Tps[0],Td0=d4f.Td,p0=d4f.Tps[1],q0=phy.q(phy.e(Td0),p0)
				...
				if(cp!=null){
					...
					var t=cp[0],T=phy.T(t),p=cp[1],ts=phy.t(phy.theta_T(ccl_theta,p0));
					//p_CCL
			...
			function calc_cp_ccl(when_inversion){	//有多个变量跟父环境共享，修改要小心！！！
				...
				if(cp!=null){
					var len=cp.length,tb=cp[len-2],pb=cp[len-1];
					cp=[tb,pb];
					...
					if(when_inversion!="skip" && isInversion(tp_series,pb)){
						for(var i=2,len=tp_series.length;i<len && tp_series[i+1]>pb;i+=2){
							var t=tp_series[i],T=phy.T(t),p=tp_series[i+1],theta=phy.theta(T,p);
							if(theta>ccl_theta){
								ccl_theta=theta;
								n=i;
							}
					...
					if(n>0){//找到最大逆温层顶
						var t=tp_series[n],T=phy.T(t),p=tp_series[n+1];
			...
			function isInversion(points,maxp){
				var pre_t=Number.MAX_VALUE,pre_p=Number.MAX_VALUE;
				for(var i=0,len=points.length;i<len && points[i+1]>maxp;i+=2){
					var t=points[i],p=points[i+1];
		...
		,draw:function(){
			...
			for(var t=0;t>=-50;t-=10){
				...
				for(var i=0,len=ys.length;i<len;i++){
					var p=ys[i],l_tmin,l_tmax,l_ta,tAlign;
...
//==================================================== 比湿曲线插件 ====================================================
	var QCurve=create({
		...
		,draw:function(){//描绘曲线
			//this.canv,this.dataMnt由外部设置
			var canv=this.canv,ts=this.getSettings(),ls=this.dataMnt.getSeries('t&td',true),L=[];
			for(var i=0,len=ls.length;i<len;i+=3){
				var T=phy.T(ls[i]),Td=phy.T(ls[i+1]),p=ls[i+2];
...
//==================================================== V-3θ曲线插件 ====================================================
	var V_3theta=create({
		...
		,draw:function(){//描绘曲线
			//this.canv,this.dataMnt由外部设置
			var canv=this.canv,ctx=canv.getContext(),ts=this.getSettings()
				,core_radius=0,shaftLength=ts.shaftLength,ctx_opt={strokeStyle:ts.color_wind}
				,ls2=this.dataMnt.getSeries('t&td',true),ls3=this.dataMnt.getSeries('w&t',true)
				,L=[],L_sed=[],L_ast=[],S_winds=[];
			for(var i=0,len=ls2.length;i<len;i+=3){
				var T=phy.T(ls2[i]),Td=phy.T(ls2[i+1]),p=ls2[i+2];
				L_sed.push(phy.t(phy.thetase(T,Td,p)),p);
			}
			for(var i=0,len=ls3.length;i<len;i+=4){
				var wd=ls3[i],wf=ls3[i+1],T=phy.T(ls3[i+2]),p=ls3[i+3],thetaast=phy.t(phy.thetase(T,T,p));
			...
			for(var i=0,len=S_winds.length;i<len;i+=4){
				fs121.wsymbol.drawWind(ctx,S_winds[i],S_winds[i+1],{x:S_winds[i+2],y:S_winds[i+3]},core_radius,shaftLength,false,ctx_opt);
			}
		}
	});
//==================================================== 帮助函数 ====================================================
	...
	function thetase_label(Thetase,thetase_points){
		var _color_moistAdiabat="#3366FF"
			,n4=thetase_points.length-6,point_t=thetase_points[n4],point_p=thetase_points[n4+1]	//取某个节点
	...
	function line_label(point_t,point_p,ar,prefix,ctx_opt){
		...
		function createLabel(ar,prefix){
			...
			for(var i=0,len=ar.length;i<len;i++){
				var name=ar[i][0],num=ar[i][1],f=ar[i][2],unit=ar[i][3];
				if(i>0)
					label+=","
